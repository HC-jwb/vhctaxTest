...
public interface FuelStatisticsRepository extends JpaRepository<FuelStatistics, Long>{
	@Query(
		value="select f.stat_dt statDate, f.tracker_id trackerId, round(f.max,2) fuelEnd, round(f.min,2) fuelStart, round(m.max,2) mileageEnd, round(m.min,2) mileageStart, \r\n" + 
		...
		"where f.report_id=:reportId and f.tracker_id=:trackerId and f.type='F' and m.type='M' and f.min is not null and (f.max - f.min) !=0\r\n" + 
		"and m.min is not null and m.max is not null order by f.tracker_id, f.raw_dt"
	, nativeQuery=true)
